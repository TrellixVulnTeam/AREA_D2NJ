...
from pytorch_pretrained_bert_self.modeling import BertForQuestionAnswering, BertForQuestionAnswering_withIfTriggerEmbedding
...
def read_query_templates(normal_file, des_file):
    ...
    with open(normal_file, "r", encoding='utf-8') as f:
        for line in f:
            ...
            # 1 template event_type + arg_name + in trigger (replace [trigger] when forming the instance)
            query_templates[event_type][arg_name].append(event_type + ' ' + arg_name + ' with external knowledge [args]' + " in [trigger]")
...
def main(args):
    ...
    if args.do_train:
        ...
        for lr in lrs:
            if not args.add_if_trigger_embedding:
                ...
                model = BertForQuestionAnswering_withIfTriggerEmbedding.from_pretrained(args.model,
            ...
            for epoch in range(int(args.num_train_epochs)):
                ...
                for step, batch in enumerate(train_batches):
                    ...
                    if not args.add_if_trigger_embedding:
                        ...
                        loss = model(input_ids=input_ids, if_trigger_ids=if_trigger_ids,
                                     token_type_ids=segment_ids, attention_mask=input_mask,
                    ...
                    if (step + 1) % eval_step == 0:
                        ...
                        if args.do_eval:
                            ...
                            result, preds = evaluate(args, model, device, eval_dataloader, eval_examples, gold_examples,
                                                     eval_features)
                            # import ipdb; ipdb.set_trace()
                            model.train()
    ...
    #         for key in result:
    #             writer.write("%s = %s\n" % (key, str(result[key])))
    #     with open(os.path.join(args.output_dir, "stu_arg_predictions.txt"), "w") as writer:
    #         for key in preds:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_dir", default='./RAMS_teacher_uncased_fenci', type=str,
                        help="The output directory where the model checkpoints and predictions will be written.")

    parser.add_argument("--train_file", default='../proc/data/RAMS-event/processed-data/RAMS_out_fenci/train_data_fenci.json',
                        type=str)
    parser.add_argument("--dev_file", default='../proc/data/RAMS-event/processed-data/RAMS_out_fenci/test_data_fenci.json', type=str)
    parser.add_argument("--test_file", default='../proc/data/RAMS-event/processed-data/RAMS_out_fenci/test_data_fenci.json', type=str)
    parser.add_argument("--gold_file", default='../proc/data/RAMS-event/processed-data/RAMS_out_fenci/test_data_fenci.json', type=str)
    ...
    parser.add_argument("--nth_query", default=1, type=int, help="use n-th template query")
    # parser.add_argument("--stu_nth_query", default=1, type=int, help="use n-th template query")
    ...
    parser.add_argument("--add_if_trigger_embedding", action='store_true', default=True,
