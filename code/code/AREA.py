...
from pytorch_pretrained_bert_self.modeling_AREA import BertForQuestionAnswering, BertForQuestionAnswering_withIfTriggerEmbedding_golden_args, \
    ...
    for (example_id, example) in enumerate(examples):
        for event in example.events:
            ...
            for argument_type in query_templates[event_type]:
                ...
                # add oracle knowledge to question query
                for arg in arguments:
                    gold_arg_type = arg[2]
                    if gold_arg_type == argument_type:
                        # exclude the argument self
                ...
                # When the argument is not in the ground truth
...
def get_distance_loss(student_embedding, teacher_embedding, t_arg_mask_metrix, s_arg_mask_metrix):
    ...
    # since teacher need to have the same related arguments as student
...
def distillation(y_s, y_e, y_s_masked, y_e_masked, teacher_scores_s1,teacher_scores_e1,
    ...
    # when teacher do not include oracle knowledge, stop RKD
...
def main(args):
    ...
    if args.do_train:
        ...
        for arg_proportion in arg_prop_list:
            ...
            # random sample the samples that contain oracle knowledge
