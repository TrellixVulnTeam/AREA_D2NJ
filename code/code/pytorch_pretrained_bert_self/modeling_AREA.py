...
class BertForQuestionAnswering_withIfTriggerEmbedding_golden_args(BertPreTrainedModel):
    ...
    def forward(self, input_ids, if_trigger_ids, token_type_ids=None, attention_mask=None,
        ...
        if start_positions is not None and end_positions is not None:
            ...
            # element in output_features indicate argument max pooling represent，5 represents an event with up to 5 argument types
            ...
            for batch_i in range(sequence_output.shape[0]):
                for arg_i, (arg_start, arg_end) in enumerate(golden_args[batch_i]):
                    # Only the small matrix in the upper left corner of the large matrix is useful
                    if distance_metric == 'angle':
                        if arg_start == 0 and arg_end == 0:
